...
contract Dao is DaoInterface, Initializable {
    ...
    mapping(address => DaoInterface.MembershipRequestStatusCode) public membershipRequestStatus;
    ...
    function vote(uint256 _campaignId, bool _agree) external onlyDaoMember {
    ...
    function voteEnd(uint256 _campaignId) public {
        require(voteInProgress[_campaignId], "No vote in progress for this campaign.");

        uint256 totalVotes = voteCountYes[_campaignId] + voteCountNo[_campaignId];
        uint256 totalMembers = daoMemberList.length;
        uint256 agreePercentage = (voteCountYes[_campaignId] * 1e18) / totalMembers;
        ...
        if (agreePercentage >= 70 * 1e16) {
            // If agreement percentage is 70% or more, claim the campaign
            donation.claim(_campaignId);
            result = true;
            message = "The campaign has been approved for claim.";
        } else {
            // If agreement percentage is less than 70%, decline the campaign
            result = false;
    ...
    function removeDaoMembership(address _user) external onlyAdmin {
        require(isDaoMember[_user], "User is not a DAO member");

        isDaoMember[_user] = false;
        membershipRequestStatus[_user] = MembershipRequestStatusCode.DEFAULT;
